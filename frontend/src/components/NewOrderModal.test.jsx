...
// jsdom doesn't implement the browser behavior where clicking a
// type="submit" button fires the form's submit event, so tests trigger
// submission directly on the form rather than via user.click on the button.
...
describe("NewOrderModal", () => {
  it("computes the running total as items are added", async () => {
    ...
    await user.selectOptions(screen.getByLabelText("Product"), "1"); // Rudraksha @ 499
  ...
  it("blocks submission when the requested quantity exceeds available stock", async () => {
    ...
    await user.selectOptions(screen.getByLabelText("Customer"), "1");
    await user.selectOptions(screen.getByLabelText("Product"), "2"); // idol, only 3 in stock
    const qtyInput = screen.getByLabelText("Qty");
    await user.clear(qtyInput);
