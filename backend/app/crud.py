...
def create_order(db: Session, order_in: schemas.OrderCreate) -> dict:
    ...
    # Lock the rows we're about to deduct stock from so two simultaneous
    # orders can't both pass the stock check and oversell the same item.
    ...
    # Validate everything before changing any stock, so a bad item doesn't
    # leave previous items half-deducted.
...
def cancel_order(db: Session, order_id: int) -> None:
    ...
    # Restore stock for a cancelled order rather than deleting history outright.
    for item in order.items:
